...
// node_modules/@auth0/auth0-angular/fesm2015/auth0-auth0-angular.mjs
...
var Auth0ClientFactory = class {
  static createClient(configFactory) {
    ...
    return new te(Object.assign(Object.assign({}, config), {
      auth0Client: {
        name: useragent.name,
...
var AuthState = class {
  constructor(auth0Client) {
    ...
    this.isLoadingSubject$ = new import_rxjs.BehaviorSubject(true);
    this.refresh$ = new import_rxjs.Subject();
    this.accessToken$ = new import_rxjs.ReplaySubject(1);
    this.errorSubject$ = new import_rxjs.ReplaySubject(1);
    this.isLoading$ = this.isLoadingSubject$.asObservable();
    this.accessTokenTrigger$ = this.accessToken$.pipe((0, import_operators.scan)((acc, current) => ({
    ...
    this.isAuthenticatedTrigger$ = this.isLoading$.pipe((0, import_operators.filter)((loading) => !loading), (0, import_operators.distinctUntilChanged)(), (0, import_operators.switchMap)(() => (
      // To track the value of isAuthenticated over time, we need to merge:
      //  - the current value
      //  - the value whenever the access token changes. (this should always be true of there is an access token
      //    but it is safer to pass this through this.auth0Client.isAuthenticated() nevertheless)
      //  - the value whenever refreshState$ emits
      (0, import_rxjs.merge)((0, import_rxjs.defer)(() => this.auth0Client.isAuthenticated()), this.accessTokenTrigger$.pipe((0, import_operators.mergeMap)(() => this.auth0Client.isAuthenticated())), this.refresh$.pipe((0, import_operators.mergeMap)(() => this.auth0Client.isAuthenticated())))
    )));
    this.isAuthenticated$ = this.isAuthenticatedTrigger$.pipe((0, import_operators.distinctUntilChanged)(), (0, import_operators.shareReplay)(1));
    this.user$ = this.isAuthenticatedTrigger$.pipe((0, import_operators.concatMap)((authenticated) => authenticated ? this.auth0Client.getUser() : (0, import_rxjs.of)(null)), (0, import_operators.distinctUntilChanged)());
    this.idTokenClaims$ = this.isAuthenticatedTrigger$.pipe((0, import_operators.concatMap)((authenticated) => authenticated ? this.auth0Client.getIdTokenClaims() : (0, import_rxjs.of)(null)));
...
var AuthService = class {
  constructor(auth0Client, configFactory, navigator, authState) {
    ...
    this.appStateSubject$ = new import_rxjs.ReplaySubject(1);
    ...
    const checkSessionOrCallback$ = (isCallback) => (0, import_rxjs.iif)(() => isCallback, this.handleRedirectCallback(), (0, import_rxjs.defer)(() => this.auth0Client.checkSession()));
    this.shouldHandleCallback().pipe((0, import_operators.switchMap)((isCallback) => checkSessionOrCallback$(isCallback).pipe((0, import_operators.catchError)((error) => {
    ...
    }), (0, import_operators.takeUntil)(this.ngUnsubscribe$)).subscribe();
  }
  /**
   ...
   */
  loginWithRedirect(options) {
    return (0, import_rxjs.from)(this.auth0Client.loginWithRedirect(options));
  }
  /**
   * ```js
  ...
  loginWithPopup(options, config) {
    return (0, import_rxjs.from)(this.auth0Client.loginWithPopup(options, config).then(() => {
   ...
   */
  logout(options) {
    return (0, import_rxjs.from)(this.auth0Client.logout(options).then(() => {
  ...
  getAccessTokenSilently(options = {}) {
    return (0, import_rxjs.of)(this.auth0Client).pipe((0, import_operators.concatMap)((client) => options.detailedResponse === true ? client.getTokenSilently(Object.assign(Object.assign({}, options), {
      detailedResponse: true
    })) : client.getTokenSilently(options)), (0, import_operators.tap)((token) => {
      if (token) {
  ...
  getAccessTokenWithPopup(options) {
    return (0, import_rxjs.of)(this.auth0Client).pipe((0, import_operators.concatMap)((client) => client.getTokenWithPopup(options)), (0, import_operators.tap)((token) => {
      if (token) {
  ...
  handleRedirectCallback(url) {
    return (0, import_rxjs.defer)(() => this.auth0Client.handleRedirectCallback(url)).pipe((0, import_operators.withLatestFrom)(this.authState.isLoading$), (0, import_operators.tap)(([result, isLoading]) => {
      var _a;
      if (!isLoading) {
        this.authState.refresh();
      }
    ...
    }), (0, import_operators.map)(([result]) => result));
  }
  shouldHandleCallback() {
    return (0, import_rxjs.of)(location.search).pipe((0, import_operators.map)((search) => {
...
var AuthGuard = class {
  ...
  canLoad(route, segments) {
    return this.auth.isAuthenticated$.pipe((0, import_operators.take)(1));
  }
  ...
  redirectIfUnauthenticated(state) {
    return this.auth.isAuthenticated$.pipe((0, import_operators.tap)((loggedIn) => {
      if (!loggedIn) {
...
var waitUntil = (signal$) => (source$) => source$.pipe((0, import_operators.mergeMap)((value) => signal$.pipe((0, import_operators.first)(), (0, import_operators.mapTo)(value))));
var AuthHttpInterceptor = class {
  ...
  intercept(req, next) {
    ...
    const isLoaded$ = this.authService.isLoading$.pipe((0, import_operators.filter)((isLoading) => !isLoading));
    return this.findMatchingRoute(req, config.httpInterceptor).pipe((0, import_operators.concatMap)((route) => (0, import_rxjs.iif)(
      // Check if a route was matched
      () => route !== null,
      // If we have a matching route, call getTokenSilently and attach the token to the
      // outgoing request
      (0, import_rxjs.of)(route).pipe(waitUntil(isLoaded$), (0, import_operators.pluck)("tokenOptions"), (0, import_operators.concatMap)((options) => this.getAccessTokenSilently(options).pipe((0, import_operators.catchError)((err) => {
        if (this.allowAnonymous(route, err)) {
          return (0, import_rxjs.of)("");
        }
        this.authState.setError(err);
        return (0, import_rxjs.throwError)(err);
      }))), (0, import_operators.switchMap)((token) => {
   ...
   */
  getAccessTokenSilently(options) {
    return (0, import_rxjs.of)(this.auth0Client).pipe((0, import_operators.concatMap)((client) => client.getTokenSilently(options)), (0, import_operators.tap)((token) => this.authState.setAccessToken(token)), (0, import_operators.catchError)((error) => {
      this.authState.refresh();
      return (0, import_rxjs.throwError)(error);
    }));
  }
  ...
  findMatchingRoute(request, config) {
    return (0, import_rxjs.from)(config.allowedList).pipe((0, import_operators.first)((route) => this.canAttachToken(route, request), null));
  }
